...
def main(
    ...
    output_folder = os.path.join(pretrained_model_path, 'results_my')
    if fast:
        save_name_1 = os.path.join(output_folder, 'inversion_fast.gif')
        save_name_2 = os.path.join(output_folder, '{}_cfg_fast_npi_prox.gif'.format(save_name))
    else:
        save_name_1 = os.path.join(output_folder, 'inversion.gif')
        save_name_2 = os.path.join(output_folder, '{}_cfg.gif'.format(save_name))
    if blend_word:
    ...
    class NullInversion:
                ...
                for rid in range(25):
                    with torch.enable_grad():
                        ...
                        if loss < 1e-6 + (50-i) * 2e-6:
                            break
                        # if target_loss.item() < 5e-5 and i > 30:
                        #     break
        ...
        def invert(self, image_path: str, prompt, offsets=(0,0,0,0), num_inner_steps=10, early_stop_epsilon=1e-5, verbose=False):
            ...
            return (image_gt, image_rec), ddim_latents[-1], uncond_embeddings, ddim_latents

        def invert_(self, image_path: str, prompt, offsets=(0,0,0,0), num_inner_steps=10, early_stop_epsilon=1e-5, verbose=False):
            self.init_prompt(prompt)
            ptp_utils.register_attention_control(self.model, None)
            image_gt = load_512_seq(image_path, *offsets)
            if verbose:
            ...
            uncond_embeddings = self.null_optimization(ddim_latents, 0, early_stop_epsilon)
    ...
    if fast:
        (image_gt, image_enc), x_t, uncond_embeddings, ddim_latents = null_inversion.invert_(image_path, prompts, offsets=(0,0,0,0), verbose=True)
    else:
        (image_gt, image_enc), x_t, uncond_embeddings, ddim_latents = null_inversion.invert(image_path, prompts, offsets=(0,0,0,0), verbose=True)
    ...
    with torch.no_grad():
        sequence = ldm_stable(
            ...
            ref_intermediate_latents=ddim_latents,
        ).videos
...
if __name__ == "__main__":
    ...
    main(**OmegaConf.load(args.config), video_len=args.video_len, fast=args.fast)
