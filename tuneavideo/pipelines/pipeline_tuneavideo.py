        ...
        **kwargs,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                latents = proximal_guidance_diffusion_step(self.unet, self.scheduler, controller, latents, text_embeddings, t, guidance_scale, low_resource=False,
                                           edit_stage=True, prox="l0", quantile=0.5,
                                           image_enc=None, recon_lr=1, recon_t=400,
                                           inversion_guidance=True, x_stars=ref_intermediate_latents, i=i, dilate_mask=1,
                                           extra_step_kwargs=extra_step_kwargs, device=device)
...
def proximal_guidance_diffusion_step(unet, scheduler, controller, latents, context, t, guidance_scale, low_resource=False,
                   edit_stage=True, prox=None, quantile=0.7,
                   image_enc=None, recon_lr=0.1, recon_t=400,
                   inversion_guidance=False, x_stars=None, i=0,
                   dilate_mask=0, extra_step_kwargs=None, device=None):
    ...
    if mask_edit is not None and inversion_guidance and (recon_t > 0 and t < recon_t) or (recon_t < 0 and t > -recon_t):
        ...
        # import numpy as np
        # img = mask_edit[1].mean(axis=0)
        # img = (img - img.min()) / (img.max() - img.min() + 1e-8)
        # img = Image.fromarray((img.detach().cpu().numpy() * 255).astype(np.uint8)).resize((256, 256))
        # img.save('img.png')
...
def dilate(image, kernel_size, stride=1, padding=0):
    ...
    dilated_image = F.max_pool3d(image, kernel_size, stride, padding)
